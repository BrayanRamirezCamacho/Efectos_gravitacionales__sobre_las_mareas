...
CONTAINS
  ...
  !!Para el eje x:
  FUNCTION fx(x,r1,m1,r2,m2)
    REAL(d),INTENT(IN)::x,r1,m1,r2,m2      !!Argumentos de la ecuación.
    REAL(d)::fx                              !!E.D a resolver.
    
    fx= -((G*m1)/r1**3)*x - G*m2*x*(r2**2+r1**2-2*x*r2)**(-3/2) + G*m2*r2*(r2**2+r1**2-2*x*r2)**(-3/2)  - ((G*m2)/r2**2)

  END FUNCTION fx

  !!Para el eje y:
  FUNCTION fy(y,x,r1,m1,r2,m2)
    REAL(d),INTENT(IN)::y,x,r1,m1,r2,m2    !!Argumentos de la ecuación.
    REAL(d)::fy                              !!E.D a resolver.

    fy = -((G*m1)/r1**3)*y -G*m2*y*(r2**2+r1**2-2*x*r2)**(-3/2)
  
  END FUNCTION fy

  !!Para el eje z:
  FUNCTION fz(z,x,r1,m1,r2,m2)
    REAL(d),INTENT(IN)::z,x,r1,m1,r2,m2    !!Argumentos de la ecuación.
    REAL(d)::fz                              !!E.D a resolver.

    fz = -((G*m1)/r1**3)*z -G*m2*z*(r2**2+r1**2-2*x*r2)**(-3/2)
...
PROGRAM mareas
  ...
  REAL(d),ALLOCATABLE::rt(:)                            !!Vector de posición en un instante t.
  ...
  REAL(d)::a
  CHARACTER (len=15)::nombre_archivo
...
!  lun%r = 1737.1E3	!!r2 
  ...
  !!MODIFICAR EL SEGMENTO ANTERIOR PARA
  !!CALCULAR A PARTIR DE LATITUD Y LONGITUD
  ...
  DO i=1,sim%n
     ...
     s = l - tr%r
     ru = rt/l
     fm(0) = fx(ru(0)*s,l,tr%m,lun%r,lun%m)!*tr%m
     fm(1) = fy(ru(1)*s,ru(0)*s,l,tr%m,lun%r,lun%m)!*tr%m
     fm(2) = fz(ru(2)*s,ru(0)*s,l,tr%m,lun%r,lun%m)!*tr%m
...
PRINT*, "Escriba el nombre del archivo que se generará con los resultados:"
READ*, nombre_archivo

   !Escribimos los valores en un archivo:
     OPEN(2, FILE=nombre_archivo, STATUS="UNKNOWN", ACTION="WRITE")
     DO i=0,sim%n
